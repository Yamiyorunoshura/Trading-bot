...
const EnhancedRiskManagement: React.FC<EnhancedRiskManagementProps> = ({ 
  ...
  const handleResolveAlert = async (alertId: string) => {
    try {
      ...
      notification.error({ message: '解決警報失敗', description: (error as Error).message })
    }
  }
  ...
  const handleEmergencyStop = async () => {
    try {
      ...
      notification.error({ message: '緊急停止失敗', description: (error as Error).message })
    }
  }
  ...
  const handleResetRiskManagement = async () => {
    try {
      ...
      notification.error({ message: '重置失敗', description: (error as Error).message })
    }
  }
